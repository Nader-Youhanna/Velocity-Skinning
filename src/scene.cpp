...
numarray<numarray<float> > compute_skinning_weights_generic(cgp::mesh const& m, skeleton_structure const& skeleton, float power_factor) 
{
	...
	for(int k_vertex=0; k_vertex<N_vertex; ++k_vertex) {
		...
		{
			// for each joint
			vec3 joint = skeleton.joint_matrix_global_bind_pose[k_joint].get_block_translation();
			// get the array of children of the joint
			numarray<int> children = skeleton.child(k_joint);
			// define d_ij
			float d_ij = DBL_MAX;
			for (int k_child = 0; k_child < children.size(); k_child++)
			{
				// for each child
				vec3 child_joint = skeleton.joint_matrix_global_bind_pose[children[k_child]].get_block_translation();
				
				// calculate distance from vertex to bone
				float dist_to_bone;
				vec3 bone = joint - child_joint;
				vec3 temp = joint - vertex;
				if (dot(temp, bone) <= 0.)
				{
					dist_to_bone = norm(temp);
				}
				else
				{
					vec3 temp2 = child_joint - vertex;
					if (dot(temp2, temp) >= 0.)
					{
						dist_to_bone = norm(temp2);
					}
					else
					{
						dist_to_bone = norm(cross(bone, temp)) / norm(bone);
					}
				}
				if (dist_to_bone < d_ij)
				{
					d_ij = dist_to_bone;
				}
			}
			sum_d_ij += 1. / pow(d_ij, power_factor);
			weights[k_vertex][k_joint] = 1. / pow(d_ij, power_factor);
		}
		for (int k_joint = 0; k_joint < N_joint; ++k_joint)
		{
			weights[k_vertex][k_joint] /= sum_d_ij;
		}
	}
